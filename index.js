...
app.post('/webhook', async (req, res) => {
  try {
    ...
    const DEBOUNCE_WAIT = parseFloat(process.env.DEBOUNCE_WAIT_SECONDS || '1.5');
    ...
    if (isImage) {
      ...
      try {
        ...
        // Download de mídia do Evolution API
        ...
        console.error('Falha no processamento de estampa multimodal:', err.message);
    ...
    if (redis && redis.status === 'ready') {
      try {
        ...
        console.error('Falha ao ler lock do Redis:', err.message);
    ...
    if (redis && redis.status === 'ready') {
      try {
        ...
        console.error('Falha ao gravar lock no Redis:', err.message);
    ...
    if (pool) {
      try {
        ...
        console.error('Falha ao recuperar histórico do Postgres:', err.message);
    ...
    if (EVOLUTION_URL && EVOLUTION_KEY) {
      try {
        ...
        console.error('Erro ao enviar mensagem pelo Evolution:', err.message);
    ...
    if (agentReply.includes('📋 **Resumo da Solicitação:**') || agentReply.toLowerCase().includes('finalizando seu atendimento')) {
      ...
      if (ai) {
        try {
          ...
          console.error('Erro ao processar JSON estruturado de lead com o Gemini:', err.message);
        }
      }
    ...
    console.error('Erro fatal na escuta do webhook:', error.message);
